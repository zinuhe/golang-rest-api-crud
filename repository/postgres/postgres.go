...
import (
	...
	repo "github.com/zinuhe/golang-rest-api-crud/repository"
...
func (r *repository) FindByID(id int) (*repo.UserModel, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, firstname, lastname, age FROM users WHERE id = ?", id).Scan(&user.Id, &user.Firstname, &user.Lastname, &user.Age)
...
func (r *repository) Find() ([]*repo.UserModel, error) {
	...
	rows, err := r.db.QueryContext(ctx, "SELECT id, firstname, lastname, age FROM users")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *repository) Create(user *repo.UserModel) error {
	...
	query := "INSERT INTO users (id, firstname, lastname, age) VALUES (?, ?, ?, ?)"
	...
	_, err = stmt.ExecContext(ctx, user.Id, user.Firstname, user.Lastname, user.Age)
	return err
}
...
func (r *repository) Update(user *repo.UserModel) error {
	...
	_, err = stmt.ExecContext(ctx, user.Firstname, user.Lastname, user.Age, user.Id)
	return err
}
